...
describe("Player methods work correctly", () => {
  ...
  describe("Base Player class methods", () => {
    ...
    it("setNextPlayer sets store.nextPlayer", () => {
      const player = new Player();
    ...
    it("getTeam returns player team", () => {
      ...
      const expected1 = "O";
      const expected2 = "X";
      const result1 = player.getTeam(store);
      const result2 = cpu.getTeam(store);
    ...
    it("getOtherPlayerTeam returns the other player's team", () => {
      ...
      const expected = "X";
      const result = player.getOtherPlayerTeam(store);
      expect(result).toEqual(expected);
    });
  ...
  describe("HumanPlayer class methods", () => {
    ...
    it("setGridChoice sets grid choice", async () => {
      ...
      await player.setGridChoice(store, 5);
      const gridResult = store.grid["5"];
      const choiceResult = store.choices;
  ...
  describe("CpuPlayer class methods", () => {
    ...
    it("getHumanPlayer gets human player from store", () => {
      const player = new HumanPlayer();
      const cpu = new CpuPlayer();
      const store = new Store();
      store.players = {
      ...
      const result = cpu.getHumanPlayer(store);
      expect(result).toEqual(expected);
    });
    ...
    it("getGridChoice returns a grid choice", async () => {
      ...
      const choice = await store.players.X.getGridChoice(store);
    ...
    it("setGridChoice sets grid choice", async () => {
      ...
      const gridExpected = "O";
      const choiceExpected = [5];
      await cpu.setGridChoice(store, 5);
      const gridResult = store.grid["5"];
      const choiceResult = store.choices;
      expect(gridResult).toEqual(gridExpected);
      expect(choiceResult).toEqual(choiceExpected);
