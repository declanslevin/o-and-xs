...
// const sendGridToBrowser = async (store, grid) => {
//   let gridObj = {
//     grid: grid,
//     id: "grid",
//   };

//   store.ws.send(JSON.stringify(gridObj));
//   // store.ws.send(grid);
// };

// const sendCurrentPlayerToBrowser = async (store, val) => {
//   let currentPlayerObj = {
//     currentPlayer: bool,
//     id: "currentPlayer",
//   };

//   store.ws.send(JSON.stringify(currentPlayerObj));
// };

const userTurn = async (store) => {
  const userChoice = await getUserGridChoice(store);
  store.setUserGridChoice(userChoice);
  // sendGridToBrowser(store, userChoice);
};
